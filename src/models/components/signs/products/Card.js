...
import CardInfo from 'models/components/signs/products/CardInfo';
import Product from 'models/components/signs/products/Product';
...
export default class Card {
    ...
    set hasLogo(val) {
        if (!Util.isBoolean(val)) {
            ErrorUtil.raiseInvalidPropertyValueError('Card', 'hasLogo', ErrorUtil.MSG.valueType.bool);
        }
    ...
    set isBkgdTransp(val) {
        if (!Util.isBoolean(val)) {
            ErrorUtil.raiseInvalidPropertyValueError('Card', 'isBkgTransp', ErrorUtil.MSG.valueType.bool);
        }
    ...
    set title(val) {
        if (!Util.isStringOrNull(val)) {
            ErrorUtil.raiseInvalidPropertyValueError('Card', 'title', ErrorUtil.MSG.valueType.stringOrNull);
        }
    ...
    set priceColCnt(val) {
        if (!Util.isNonNegInt(val)) {
            ErrorUtil.raiseInvalidPropertyValueError('Card', 'priceColCnt', ErrorUtil.MSG.valueType.nonNegInt);
        }
    ...
    static fromJSON(json) {
        ...
        obj._disclaimers = ('disclaimers' in json && Array.isArray(json['disclaimers'])) ? json['disclaimers'].map((d) => d) : [];
